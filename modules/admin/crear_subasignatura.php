...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $horas_fg = is_numeric($_POST['horas_frente_grupo']) ? $_POST['horas_frente_grupo'] : 0;
    $horas_ind = is_numeric($_POST['horas_independiente']) ? $_POST['horas_independiente'] : 0;
    $creditos = is_numeric($_POST['creditos']) ? $_POST['creditos'] : 0;
    ...
    if (!$error) {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM subasignaturas WHERE clave = ?");
        $stmt->execute([$clave]);

        if($stmt->fetchColumn() > 0){
            $error = "Ya existe una subasignatura con esa clave.";
        }
    }
    ...
    if (!$error) {
        try {
            ...
            echo "<script>
                alert('Subasignatura registrada correctamente');
...
<div class="max-w-7xl mx-auto p-6 bg-slate-50 min-h-screen animate-fade-in">
    ...
    <div class="mb-10">
        ...
        <p class="text-purple-600 font-bold text-[10px] uppercase tracking-[0.3em] italic mt-2">
            Panel de Alta de Subasignaturas
        </p>
    </div>
    ...
    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 mb-12 relative overflow-hidden">
        ...
        <form method="POST" class="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div class="md:col-span-3">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Clave</label>
            ...
            <div class="md:col-span-9">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Nombre de la Subasignatura</label>
            ...
            <div class="md:col-span-3">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Créditos</label>
            ...
            <div class="md:col-span-12">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Descripción</label>
