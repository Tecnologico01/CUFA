...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    try{
        ...
        $pdo->commit();
        echo "<script>alert('Materia actualizada correctamente'); window.location.href='/sistema_academico/dashboards/coordinador_dashboard.php?modulo=materias';</script>";
        exit;
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-slate-900 p-8 rounded-[2.5rem] shadow-xl overflow-hidden relative">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest italic border-b border-slate-800 pb-2">Sub-asignaturas</h3>
                    <div class="flex gap-2">
                        <button type="button" onclick="toggleSubSelector(true)" class="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Vincular</button>
                        <button type="button" onclick="toggleSubSelector(false)" class="bg-slate-800 hover:bg-slate-700 text-slate-400 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Cerrar</button>
                    </div>
                </div>
...
/* TOGGLE SELECTOR */
function toggleSubSelector(show){
